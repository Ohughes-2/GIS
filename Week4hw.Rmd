...
## I switched the order of the join data frames to maintain the geometry so this is pointless
## good to know tho
```
```{r}
...
```{r}
tmap_mode('view')
gen_ineq_tmap <- tm_shape(gen_ineq_paices2) + 
  tm_polygons(fill = "difference", 
              fill.scale = tm_scale(
                            values = c('#3c0008', '#cc7070', '#ff69b4'),   
                            midpoint = 0.1), 
              fill_alpha = 0.8, 
              fill.legend = tm_legend(title = "[2010-2019] GII")) + 
  tm_title("Difference in Gender Inequality Index from 2010–2019") + 
  tm_credits( "Source: UN Human Development Reports", position = c("left", "bottom")) +
  tm_scalebar(position = c("left", "bottom")) +
  tm_view(set_view = c(0, 20, 1.4)) 

gen_ineq_tmap
```
HW5 Author: Nathaniel H 

Map where darker blue is greater positive change in GII between 2010 and 2019. Redder is negative. White is no change
```{r}
tm1 <- tm_shape(gen_ineq_paices2) + 
  tm_polygons(fill ="difference",
              col = "black", 
              lwd =0.25,
              lty="solid",
...
I liked your notes a lot while i ran your code (it was entertaining and informative). Your code ran well and I liked your maps (it was all very clear).

My only tips would be that you do not need to print every comment/explanation. There are two ways to write comments in R markdown:
1) write normal text like this (no quotes no hashtags) between code blocks 
2) use one # to create a comment within the code block.

Printing the comments means that when someone runs your code it will print to the screen, which uses (albeit very little) compute power. 

I really liked your code and explanations, that is just the one small change i would suggest. Andy can probably explain it better than me.
